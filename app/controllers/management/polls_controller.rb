class Management::PollsController < Management::BaseController
  include PollsHelper
  include HasFilters
  include HasOrders
  ...
  def show
    ...
    @token = poll_voter_token(@poll, managed_user)
    @poll_questions_answers = Poll::Question::Answer.where(question: @poll.questions).where.not(description: "").order(:given_order)

    @answers_by_question_id = {}
    poll_answers = ::Poll::Answer.by_question(@poll.question_ids).by_author(managed_user.try(:id))
